...
The main thread owns product decisions, active specifications and plans, integration, and Git. Give subagents anchored briefs and disjoint write sets. Researcher, architect, and QA roles are report-only and receive no write set; their `permissionMode = plan` and restricted tools are intent, while the parent permission context can take precedence. Each write-capable role receives only its approved bounded write set. Delegation is main-mediated: every worker returns to the main thread, worker recommendations are advisory, and only the main thread selects and dispatches the next capability. Subagents never invoke the Agent tool or spawn children. When independent work is ready, the main thread may dispatch one parallel wave only with a stable contract, disjoint scopes, no result dependency, and a clear join condition; otherwise keep it serial. For Execute checkpoints, delegation is the default: when an approved project-local role can own useful bounded research, architecture, implementation, review, testing, or another capability with a clear boundary, dispatch that role before the main thread duplicates the work. Reread the selected `.claude/agents/*.md` before each wave; changed model, effort, tools, permission, or instruction values apply to new subagents, while running workers keep their creation profile. Use the current project-local values; do not invent or substitute a personal/global role. If a role is absent in the active worktree, refresh only the ignored project-local agent files from the source checkout through the repository's propagation convention, reread them, and use `Direct: <reason>` only if it remains unavailable. Record `delegation_roles` and `delegation_status` in the checkpoint resume record and restore them after compaction. Keep product decisions, lifecycle/plan state, joins, integration, Git, singleton operations, or tiny coupled actions on the main thread; otherwise a missing role is an explicit `Direct: <reason>` fallback. Report each delegation wave with role names, configured model values, effort values, and the report-only/no-write-set/no-child boundary. Claude model and effort choices are host-native starting points and remain adjustable; do not copy Codex model slugs into these files. Plumbline does not edit global Claude settings or enable experimental Agent Teams.
{AGENT_GUIDANCE_END}
"""
